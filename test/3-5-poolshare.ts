...
describe("Pool Share", async function () {
    ...
    beforeEach('Setup', async function () {
        ...
        const launchPadFactory = await ethers.getContractFactory("PoolShare");
    ...
    it ("Test Deposit", async () => {
        ...
        await test_mint(bUSDContract, addrs[1], "100000");        
        await testContract.connect(addr1).depositBusd(PoolIndex, _amountToken, pe("900")); // 90 Token sell to 900 BUSD

        await test_balance(bUSDContract, testContract, "2200.0"); // prefund: 1300 + deposit 900
    });
    ...
    it ("Test Claim", async () => {
        ...
        await test_balance(tokenContract, addr2, "0.0"); // 10%, after fee
        await test_balance(bUSDContract, addr2, "990.0"); // init 1000, prefund 100, approve 100, 10% token sell to 900 busd => claim 90 busd
        ...
        // claim agian
        await testContract.connect(addr2).claim(PoolIndex);
        await test_balance(tokenContract, addr2, "0.0"); // 
        await test_balance(bUSDContract, addr2, "1110.0"); // more 120 busd

        // Investor 1: refunded, claim only token
        await testContract.connect(addr4).claim(PoolIndex);
        await test_balance(tokenContract, addr4, "0.0"); // 10%, after fee
        await test_balance(bUSDContract, addr4, "1220.0"); // 800 + 180 + 240 
